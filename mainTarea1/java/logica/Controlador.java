...
import logica.ManejadorActividadDeportiva;
import logica.ManejadorClase;
import logica.ManejadorInstitucionDeportiva;
import logica.ManejadorProfesor;
import logica.ManejadorRegistro;
...
public class Controlador implements IControlador{
	...
	public void AltaUsuario(DtUsuario dtUser, String pass) throws UsuarioRepetidoExcepcion {
		...
		if(dtUser instanceof DtProfesor){
			...
			if(!existeMail(dtProf.getEmail()) && !existeNickname(dtProf.getNickname())) {
	...
	public DtUsuario ConsultaUsuario(String user) {
		...
		if(mp.existeNickname(user)) {
			...
			DtSocio dtSocio = new DtSocio(soc.getNickname(), soc.getNombre(),soc.getApellido(), soc.getEmail(),soc.getFecha(),soc.getProfileImage());	
	...
	public boolean ModificarDatosUsuario(DtUsuario dtUser, DtUsuario dtUserNew) {
		...
		if(dtUser instanceof DtSocio) {
			...
			s.setProfileImage(dtSocioNew.getProfileImage());
			ms.modSocio(s);
			return true;
			...
			p.setProfileImage(dtProfNew.getProfileImage());
			mp.modProfesor(p);
			return true;	
		}
		return false;
	...
	public DtActividadDeportiva ConsultaActividadDeportiva(String actDep){
		...
		if(mad.existeNombre(actDep)) {
			...
			DtActividadDeportiva dtActividadDeportiva = new DtActividadDeportiva(ad.getNombre(), ad.getDescripcion(), ad.getDuracion(), ad.getCosto(), ad.getFecha(), ad.getInstitucionDeportiva().getNombre());	
	...
	public DtInstitucionDeportiva getInstitucionDt(String nombre){
		...
		DtInstitucionDeportiva dtInst = new DtInstitucionDeportiva(instDep.getNombre(),instDep.getDescripcion(),instDep.getUrl());
		return dtInst;
	}
	...
	public DtSocio findSocio(String name) {
		...
		DtSocio dtSocio = new DtSocio(s.getNickname(),s.getNombre(),s.getApellido(),s.getEmail(),s.getFecha(),s.getProfileImage());
		return dtSocio;
	}
	...
	public ArrayList<DtRegistro> getRegistrosClase(String nomClase) {
		Clase c = mc.buscarClase(nomClase);
		List<Registro> l = c.getRegistros();
		ArrayList<DtRegistro> ldt = new ArrayList<>();
		for(Registro r: l){
			DtRegistro dtreg = new DtRegistro(r.getClase().getNombre(),r.getFecha(),r.getSocio().getNombre());
			ldt.add(dtreg);
		}
		return ldt;
	}
	...
	public ArrayList<DtClase> getClasesActDep(String nomActDep) {
		ActividadDeportiva ad = mad.buscarActividadDeportiva(nomActDep);
		List<Clase> clases = ad.getClases();
	...
	public ArrayList<DtRegistro> getRegistrosSocio(String nomSocio) {
		Socio s = ms.buscarSocio(nomSocio);
		List<Registro> regs = s.getRegistros();
		ArrayList<DtRegistro> dtRegs = new ArrayList<>();
		for(Registro r: regs){
			DtRegistro dtReg = new DtRegistro(r.getClase().getNombre(), r.getFecha(), r.getSocio().getNombre());
			dtRegs.add(dtReg);
		}
	...
	public ArrayList<DtActividadDeportiva> getActividadesInstitucion(String nomInstDep){
        InstitucionDeportiva id = mid.buscarInstitucionDeportiva(nomInstDep);
        List<ActividadDeportiva> actsDeps = id.getActividadesDeportivas();
        ArrayList<DtActividadDeportiva> dtActsDep = new ArrayList<>();
        for(ActividadDeportiva ad: actsDeps){
            DtActividadDeportiva dtActDep = new DtActividadDeportiva(ad.getNombre(), ad.getDescripcion(), ad.getDuracion(), ad.getCosto(), ad.getFecha(), ad.getInstitucionDeportiva().getNombre());
            dtActsDep.add(dtActDep);
        }
        return dtActsDep;
    }
	...
	public ArrayList<DtProfesor> getProfesoresInstitucion(String nomInstDep){
        InstitucionDeportiva id = mid.buscarInstitucionDeportiva(nomInstDep);
        List<Profesor> profs = id.getProfesores();
        ArrayList<DtProfesor> dtProfs = new ArrayList<>();
