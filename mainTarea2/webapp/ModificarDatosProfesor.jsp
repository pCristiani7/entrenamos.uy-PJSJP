...
<form action="ModificarDatosUsuario" method="post">
			<div class="container">
				<div class="card">
					...
					<img src="data:image/png;base64,<%=imgEnn%>" width="300px" height="300px" name="imagenPreCarga" id="imagenPreCarga">
					...
					<div class="inputBox">
						...
						<script>document.getElementById('1').value='<%=dtProfesor.getNombre()%>';</script>
						<span>Nombre del Profesor</span>
					</div>
					<div class="inputBox">
						<input id="2" type="text" name="apeUsuario" >
						<script>document.getElementById('2').value='<%=dtProfesor.getApellido()%>';</script>
						<span>Apellido del Profesor</span>
					</div>
					<div class="inputBox">
					    <label>Fecha de Nacimiento nueva</label>
					    <input id="3" type="date" name="fNacUsuario">
					    <script>document.getElementById('3').value='<%=dtProfesor.getFechaNac().getTime()%>';</script>
					  </div>
					 <div class="inputBox">
						<input id="4" type="text" name="descProf" >
						<script>document.getElementById('4').value='<%=dtProfesor.getDescripcion()%>';</script>
						<span>Descripcion</span>
					</div>
					<div class="inputBox">
						<input id="5" type="text" name="bioProf" >
						<script>document.getElementById('5').value='<%=dtProfesor.getBiografia()%>';</script>
						<span>Biografia</span>
					</div>
					<div class="inputBox">
						<input id="6" type="text" name="swProf" >
						<script>document.getElementById('6').value='<%=dtProfesor.getSitioweb()%>';</script>
						<span>Sitio Web</span>
					</div>
