...
public class ModificarDatosUsuario extends JInternalFrame {
	...
	protected void modificarUsuarioActionPerformed(ActionEvent arg0) {
		...
		if(dtUsr instanceof DtSocio) {
			...
			List<DtRegistro> reg = dtSOld.getDtRegistros();
			DtSocio dtSNew = new DtSocio(nickname,nombre,apellido,mail,fechaNac,reg);
			if(iCon.ModificarDatosUsuario(dtSOld, dtSNew)) {
				JOptionPane.showMessageDialog(this, "Usuario modificado con éxito!", "Modificar Usuario",
			...
			DtProfesor dtPNew = new DtProfesor(nickname,nombre,apellido,mail,fechaNac,descripcion,biografia,url,inst);
